...
if __name__ == "__main__":
	dim = 4
	hilb = numpy.zeros((dim*2+1,dim*2+1,dim*2+1))
	lastcoord = None
	...
	#	for j in range(dim):
	#		for k in range(dim):
	#			hilb[i,j,k] = Hilbert_to_int((i,j,k))
	hilb[2:2*dim-1,2:2*dim-1, 2:2*dim-1] = 0
	neg = 1 - hilb
	neg[0:1,:,:] = 0
	neg[:,0:1,:] = 0
	neg[:,:,0:1] = 0
	...
	neg = ndimage.zoom(neg, scale, order=0)
	name = "neghilb%d"%(dim)	
	mrc.write(neg, name+".mrc")
